...
def parse_plantuml(content):
    ...
    for match in re.finditer(activity_pattern, content):
        ...
        if activity_name:
            ...
            if not any(a["name"] == activity_name for a in activities):
                print(
                    colored(
                        f"Found activity: {activity_name} (Type: {node_type})", "yellow"
                    )
                )
                activities.append(
                    {"name": activity_name, "type": node_type, "description": ""}
                )
    ...
    for line in lines:
        ...
        if "->" in line:
            ...
            if any(
                ...
                for keyword in ["@startuml", "@enduml", "skinparam"]
            ):
            ...
            if len(parts) == 2:
                ...
                print(
                    colored(
                        f"Found potential transition: {source} -> {target} [{label}]",
                        "yellow",
                    )
                )
                ...
                if source in activity_names and target in activity_names:
                    ...
                    print(
                        colored(
                            f"Added transition: {source} -> {target} [{label}]", "green"
                        )
                    )
                else:
                    print(
                        colored(
                            f"Skipping invalid transition: {source} -> {target}", "red"
                        )
                    )
                    print(
                        colored(
                            f"Source in activities: {source in activity_names}",
                            "yellow",
                        )
                    )
                    print(
                        colored(
                            f"Target in activities: {target in activity_names}",
                            "yellow",
                        )
                    )
    ...
    print(
        colored(
            f"Found {len(activities)} activities and {len(transitions)} transitions",
            "yellow",
        )
    )
    ...
    async with httpx.AsyncClient() as client:
        for activity in activities:
            try:
                ...
                print(
                    colored(
                        f"✓ Created activity: {activity['name']} (Type: {activity['type']}, Id: {node_id})",
                        "green",
                    )
                )
            except Exception as e:
                print(
                    colored(
                        f"❌ Error creating activity {activity['name']}: {str(e)}",
                        "red",
                    )
                )
                continue
...
async def create_transitions(
    token: str, diagram_id: str, transitions: list, activity_ids: dict
) -> None:
    ...
    for transition in transitions:
        ...
        if not source_id or not target_id:
            print(
                colored(f"❌ Error: Could not find IDs for {source} -> {target}", "red")
            )
            continue
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                print(
                    colored(f"Response: {response.text}", "yellow")
                )  # Added for debugging
        except Exception as e:
...
async def create_activity_diagram_from_plantuml(
...
def main():
    parser = argparse.ArgumentParser(
        description="Create an activity diagram from PlantUML using STUDIO API"
    )
    parser.add_argument(
    ...
    asyncio.run(
        create_activity_diagram_from_plantuml(
