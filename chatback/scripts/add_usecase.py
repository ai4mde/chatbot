...
def parse_plantuml(content):
    ...
    for match in re.finditer(actor_pattern, content):
        ...
        actors.append({"name": actor_name, "description": ""})
    ...
    for match in re.finditer(usecase_pattern, content):
        ...
        usecases.append({"name": usecase_name, "id": usecase_id, "description": ""})
    ...
    for match in re.finditer(rel_pattern, content):
        ...
        print(
            colored(
                f"Found relationship: {source} {rel_type} {target} {label}", "yellow"
            )
        )
        relationships.append(
            {"source": source, "target": target, "type": rel_type, "label": label}
        )

    print(
        colored(
            f"Found {len(actors)} actors, {len(usecases)} use cases, and {len(relationships)} relationships",
            "yellow",
        )
    )
...
async def verify_system(token: str, project_id: str, system_id: str) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if not system:
            ...
            print(
                colored(
                    f"Please check if system ID {system_id} exists in project {project_id}",
                    "yellow",
                )
            )
            return None
    ...
    async with httpx.AsyncClient() as client:
        for actor in actors:
            try:
                ...
                print(
                    colored(f"❌ Error creating actor {actor['name']}: {str(e)}", "red")
                )
                continue
    return actor_ids
    ...
    async with httpx.AsyncClient() as client:
        for usecase in usecases:
            try:
                ...
                print(
                    colored(
                        f"❌ Error creating use case {usecase['name']}: {str(e)}", "red"
                    )
                )
                continue
    return usecase_ids
...
async def create_relationships(
    token: str, diagram_id: str, relationships: list, actor_ids: dict, usecase_ids: dict
):
    ...
    async with httpx.AsyncClient() as client:
        for rel in relationships:
            try:
                source_id = actor_ids.get(rel["source"]) or usecase_ids.get(
                    rel["source"]
                )
                target_id = actor_ids.get(rel["target"]) or usecase_ids.get(
                    rel["target"]
                )
                ...
                if source_id and target_id:
                    ...
                    edge_data = {
                        "source": source_id,
                        "target": target_id,
                        "rel": {"type": edge_type, "label": rel["label"] or ""},
                    }
                    response = await client.post(
                    ...
                    print(
                        colored(
                            f"✓ Created {edge_type} relationship: {rel['source']} -> {rel['target']}",
                            "green",
                        )
                    )
                else:
...
async def create_usecase_diagram_from_plantuml(
    ...
    try:
        ...
        await create_relationships(
            token, diagram_id, relationships, actor_ids, usecase_ids
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a use case diagram from PlantUML using STUDIO API"
    )
    parser.add_argument(
    ...
    asyncio.run(
        create_usecase_diagram_from_plantuml(
