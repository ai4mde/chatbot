...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Ensure all required data directories exist for a given group"
    )
    parser.add_argument(
        "group_name", help="Name of the group to create directories for"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Print verbose output"
    )
    parser.add_argument(
        "--data-path", help="Override the data path (default: from settings)"
    )
    ...
    paths = ensure_data_directories(
        args.group_name, base_path=data_path, verbose=args.verbose
    )
