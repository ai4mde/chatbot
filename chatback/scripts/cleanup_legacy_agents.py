...
def main():
    ...
    try:
        ...
        if not os.path.basename(current_dir) == "chatback":
            print(
                colored(
                    "Error: This script should be run from the chatback directory.",
                    "red",
                )
            )
            return
        ...
        backup_dir = os.path.join(
            current_dir,
            "backups",
            f"legacy_agents_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        )
        ...
        for file_path in LEGACY_FILES:
            ...
            if not os.path.exists(full_path):
                print(
                    colored(
                        f"Warning: File {file_path} does not exist, skipping.", "yellow"
                    )
                )
                continue
