...
def parse_plantuml(content):
    ...
    for match in re.finditer(class_pattern, content):
        ...
        for line in class_body.split("\n"):
            ...
            if "(" in line:
                ...
                print(
                    colored(
                        f"  Found method: {method_name}({params_part}) -> {return_type}",
                        "yellow",
                    )
                )
                methods.append(
            ...
            # Attribute
                ...
                if len(attr_parts) == 2:
                    ...
                    print(
                        colored(
                            f"  Found attribute: {attr_name}: {attr_type} [{visibility}]",
                            "yellow",
                        )
                    )
        ...
        classes.append(
            {"name": class_name, "attributes": attributes, "methods": methods}
        )
    ...
    for match in re.finditer(rel_pattern, content, re.MULTILINE):
        ...
        if not any(start <= pos <= end for start, end in class_positions):
            ...
            print(
                colored(
                    f"Found relationship: {source} {rel_type} {target} ({relationship_type})",
                    "yellow",
                )
            )

            # Only add if both source and target are actual classes
            if source in [cls["name"] for cls in classes] and target in [
                ...
                print(
                    colored(
                        f"Skipping relationship {source} -> {target} as one or both classes not found",
                        "yellow",
                    )
                )
    ...
    print(
        colored(
            f"Found {len(classes)} classes and {len(relationships)} relationships",
            "yellow",
        )
    )
        ...
        async with httpx.AsyncClient(follow_redirects=True) as client:
            ...
            if not project:
                ...
                for p in projects:
                    print(
                        colored(
                            f"  - ID: {p.get('id')} | Name: {p.get('name')}", "yellow"
                        )
                    )
                return None
        ...
        async with httpx.AsyncClient(follow_redirects=True) as client:
            ...
            system = next(
                (
                    s
                    for s in systems
                    if s.get("project") == project_id and s.get("id") == system_id
                ),
                None,
            )
            ...
            if not system:
                ...
                for s in project_systems:
                    print(
                        colored(
                            f"  - ID: {s.get('id')} | Name: {s.get('name')}", "yellow"
                        )
                    )
                return None
        ...
        for cls in classes:
            try:
                ...
                for attr in cls["attributes"]:
                    ...
                    print(
                        colored(
                            f"  Adding attribute: {attr['name']}: {attr_type}", "yellow"
                        )
                    )
                ...
                for method in cls["methods"]:
                    ...
                    print(
                        colored(
                            f"  Adding method: {method['name']} -> {method_type}",
                            "yellow",
                        )
                    )
                ...
                print(
                    colored(f"✓ Created class: {cls['name']} (Id: {node_id})", "green")
                )
            except Exception as e:
                print(
                    colored(f"❌ Error creating class {cls['name']}: {str(e)}", "red")
                )
                continue
    return class_ids
...
async def create_relationships(
    token: str, diagram_id: str, relationships: list, class_ids: dict
):
    ...
    async with httpx.AsyncClient() as client:
        for rel in relationships:
            try:
                ...
                print(
                    colored(
                        f"Processing relationship: {rel['source']} -> {rel['target']} ({rel['type']})",
                        "yellow",
                    )
                )
                ...
                if source_id and target_id:
                    ...
                    if response.status_code != 200:
                        print(
                            colored(
                                f"Response status: {response.status_code}", "yellow"
                            )
                        )
                    ...
                    print(
                        colored(
                            f"✓ Created {rel['type']} relationship: {rel['source']} -> {rel['target']}",
                            "green",
                        )
                    )
                else:
                    print(
                        colored(
                            f"❌ Error: Could not find IDs for {rel['source']} -> {rel['target']}",
                            "red",
                        )
                    )
            except Exception as e:
...
async def read_plantuml_file(plantuml_file: str) -> str:
    ...
    try:
        ...
        print(
            colored(f"❌ Error: File {plantuml_file} not found in {script_dir}", "red")
        )
        return None


async def create_class_diagram_from_plantuml(
    username: str, password: str, project_id: str, system_id: str, plantuml_file: str
):
    ...
    try:
        ...
        if not project:
            print(
                colored(
                    "\nTip: Use the project IDs listed above or create a new project first.",
                    "yellow",
                )
            )
            ...
            print(
                colored(
                    "  python scripts/add_project.py -u <username> -w <password> -p <project_name> -s <system_name>",
                    "yellow",
                )
            )
            return
        ...
        print(
            colored("2. Check if the project ID exists using the list above", "yellow")
        )
        print(
            colored(
                "3. Ensure the STUDIO_API_URL is correct in your environment", "yellow"
            )
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a class diagram from PlantUML using STUDIO API"
    )
    parser.add_argument(
        "-u", "--user", required=True, help="Username for STUDIO API authentication"
    )
    parser.add_argument(
        "-w", "--password", required=True, help="Password for STUDIO API authentication"
    )
    ...
    asyncio.run(
        create_class_diagram_from_plantuml(
            args.user, args.password, args.project, args.system, args.input
        )
    )
