...
async def verify_project_system(
    username: str, password: str, project_id: str, system_id: str
):
    ...
    try:
        ...
        print(
            colored(f"Project: {project.get('name', 'N/A')} ({project_id})", "yellow")
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify project and system existence in ACC API"
    )
    parser.add_argument(
        "-u", "--user", required=True, help="Username for ACC API authentication"
    )
    parser.add_argument(
        "-w", "--password", required=True, help="Password for ACC API authentication"
    )
    ...
    asyncio.run(
        verify_project_system(args.user, args.password, args.project, args.system)
    )
