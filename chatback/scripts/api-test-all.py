...
async def test_class_diagram(agent, project_id, system_id):
    ...
    try:
        ...
        diagram = await agent.create_diagram(
            system_id=system_id, plantuml_code="", diagram_type="classes"
        )
        diagram_id = diagram["id"]
        ...
        user = await agent.add_node(
            ...
            properties={
                ...
                "methods": [
                    {"name": "login", "description": "", "type": "str", "body": ""},
                    {"name": "logout", "description": "", "type": "str", "body": ""},
                    {
                        "name": "updateProfile",
                        "description": "",
                        "type": "str",
        ...
        order = await agent.add_node(
            ...
            properties={
                ...
                "methods": [
                    {"name": "place", "description": "", "type": "str", "body": ""},
                    {"name": "cancel", "description": "", "type": "str", "body": ""},
                    {"name": "getStatus", "description": "", "type": "str", "body": ""},
                ],
            },
        )
...
async def test_use_case_diagram(agent, project_id, system_id):
    ...
    try:
        ...
        diagram = await agent.create_diagram(
            system_id=system_id, plantuml_code="", diagram_type="usecase"
        )
        diagram_id = diagram["id"]
        ...
        customer = await agent.add_actor(
            diagram_id=diagram_id, name="Customer", description="A registered customer"
        )
        print(f"✅ Created Customer actor with ID: {customer['data']['id']}")

        # Create Admin actor
        admin = await agent.add_actor(
            diagram_id=diagram_id, name="Admin", description="System administrator"
        )
        ...
        login = await agent.add_use_case(
            diagram_id=diagram_id, name="Login", description="User authentication"
        )
        ...
        print(
            f"✅ Created Manage Profile use case with ID: {manage_profile['data']['id']}"
        )

        # Create View Orders use case
        view_orders = await agent.add_use_case(
            diagram_id=diagram_id, name="View Orders", description="View order history"
        )
        ...
        place_order = await agent.add_use_case(
            diagram_id=diagram_id, name="Place Order", description="Create a new order"
        )
...
async def main(username: str, password: str):
    ...
    try:
        ...
        project_id = await agent.create_project(
            name=project_name, description="Test project for UML diagrams"
        )
