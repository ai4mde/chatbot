...
def main():
    ...
    try:
        # Import the factory
        from app.services.chat.interview import (
        ...
        if is_langgraph:
            print(
                colored("✅ Success: LangGraph implementation is being used", "green")
            )
        else:
            print(
                colored("❌ Error: LangGraph implementation is NOT being used", "red")
            )

    except Exception as e:
        logger.error(
            f"Error verifying LangGraph implementation: {str(e)}", exc_info=True
        )
