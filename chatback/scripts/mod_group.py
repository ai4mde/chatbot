...
async def show_group_details(group: Group) -> None:
    ...
    print(
        f"Updated: {group.updated_at.strftime('%Y-%m-%d %H:%M:%S') if group.updated_at else 'never'}"
    )
    ...
    try:
        async with AsyncSessionLocal() as session:
            async with session.begin():
                ...
                new_description = prompt_input(
                    "New description", required=False, default=group.description or ""
                )
                ...
                while True:
                    ...
                    if choice == "1":
                        ...
                        if emails:
                            ...
                            if missing_emails:
                                print_color(
                                    f"Warning: Users not found: {', '.join(missing_emails)}",
                                    YELLOW,
                                )

                            group.users.extend(
                                [u for u in users if u not in group.users]
                            )
                            print_color(
                                f"Added {len(found_emails)} users to group", GREEN
                            )
                        ...
                        for i, user in enumerate(
                            sorted(group.users, key=lambda x: x.email), 1
                        ):
                            print(f"{i}. {user.email}")

                        to_remove = prompt_input(
                            "Enter numbers to remove (comma-separated, empty to cancel)",
                            required=False,
                        )
                        if to_remove:
                            try:
                                indices = [
                                    int(i.strip()) - 1 for i in to_remove.split(",")
                                ]
                                users_to_remove = [
                                    sorted(group.users, key=lambda x: x.email)[i]
                                    for i in indices
                                    if 0 <= i < len(group.users)
                                ]
                                for user in users_to_remove:
                                    group.users.remove(user)
                                print_color(
                                    f"Removed {len(users_to_remove)} members", GREEN
                                )
