...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["group_id"], ["groups.id"], name="fk_users_group_id_groups"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.execute(
        "CREATE TYPE conversationstate AS ENUM ('INTERVIEW', 'DOCUMENT', 'COMPLETED')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "role",
            postgresql.ENUM(
                "USER", "ASSISTANT", "SYSTEM", name="chatrole", create_type=False
            ),
            nullable=False,
        ),
