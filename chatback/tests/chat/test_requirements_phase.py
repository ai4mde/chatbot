...
async def mock_analyze_interview_markdown(state):
    ...
    state["functional_requirements"] = MOCK_REQUIREMENTS_RESULT[
        "functional_requirements"
    ]
    state["non_functional_requirements"] = MOCK_REQUIREMENTS_RESULT[
        "non_functional_requirements"
    ]
...
async def mock_save_requirements(state):
    ...
    print(
        colored(
            f"Mock save_requirements called with chat_title: {state.get('chat_name', 'unknown')}",
            "blue",
        )
    )
...
async def test_requirements_phase():
    ...
    try:
        ...
        print(
            colored(f"Testing requirements phase for session {session_id}...", "blue")
        )
        ...
        print(
            colored(
                f"Creating temporary interview file at {interview_file_path}", "blue"
            )
        )
        ...
        try:
            ...
            with (
                ...
                patch(
                    ...
                    new=mock_analyze_interview_markdown,
                ),
                patch(
                    "app.services.chat.requirements.requirements_agent_graph.save_requirements",
                    new=mock_save_requirements,
                ),
                patch(
                    "app.services.chat.requirements.requirements_agent_graph.prepare_result",
                    new=mock_prepare_result,
                ),
            ):
                ...
                print(
                    colored(
                        f"Analyzing interview markdown file for chat_title: {chat_title}",
                        "blue",
                    )
                )
                requirements_result = await requirements_agent.analyze_interview_file(
                    chat_title
                )
                ...
                print(
                    colored(
                        f"Message: {requirements_result.get('message', 'No message')}",
                        "cyan",
                    )
                )
                ...
                non_functional_reqs = requirements_result.get(
                    "non_functional_requirements", []
                )

                print(
                    colored(
                        f"Extracted {len(functional_reqs)} functional requirements:",
                        "green",
                    )
                )
                for req in functional_reqs:
                    print(
                        colored(
                            f"  - {req.get('id')}: {req.get('name')} - {req.get('description')}",
                            "green",
                        )
                    )
                ...
                print(
                    colored(
                        f"Extracted {len(non_functional_reqs)} non-functional requirements:",
                        "green",
                    )
                )
                ...
                nfr_path = requirements_result.get(
                ...
                if os.path.exists(fr_path):
                    print(
                        colored(
                            f"Functional requirements file created at: {fr_path}",
                            "green",
                        )
                    )
                    ...
                    with open(fr_path, "r", encoding="utf-8") as f:
                        print(
                            colored(f"Content of functional requirements file:", "cyan")
                        )
                        print(colored(f.read()[:200] + "...", "cyan"))
                else:
                    print(
                        colored(
                            f"Functional requirements file not created at: {fr_path}",
                            "red",
                        )
                    )
                ...
                if os.path.exists(nfr_path):
                    print(
                        colored(
                            f"Non-functional requirements file created at: {nfr_path}",
                            "green",
                        )
                    )
                    # Print the content of the file
                    with open(nfr_path, "r", encoding="utf-8") as f:
                        print(
                            colored(
                                f"Content of non-functional requirements file:", "cyan"
                            )
                        )
                        print(colored(f.read()[:200] + "...", "cyan"))
                else:
                    print(
                        colored(
                            f"Non-functional requirements file not created at: {nfr_path}",
                            "red",
                        )
                    )
