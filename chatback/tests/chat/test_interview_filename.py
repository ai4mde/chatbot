...
async def modified_analyze_interview_markdown(state):
    ...
    from app.services.chat.requirements.requirements_agent_graph import analyze_interview_markdown
    ...
    if os.path.exists(interviews_dir):
        ...
        logger.info(f"Files in {interviews_dir}: {os.listdir(interviews_dir) if os.path.exists(interviews_dir) else 'directory not found'}")
        ...
        if os.path.exists(os.path.join(interviews_dir, original_chat_title)):
            ...
            logger.info(f"Found interview file with exact original name: {original_chat_title}")
            ...
            if not interview_file:
                for filename in os.listdir(interviews_dir):
                    logger.info(f"Checking file: {filename} against chat_title: {chat_title}")
                    if filename.endswith('.md'):
                        if chat_title.lower() in filename.lower():
                            interview_file = os.path.join(interviews_dir, filename)
                            logger.info(f"Found interview file by substring match: {filename}")
                            break
                        elif chat_title.lower().startswith(filename.lower().replace('.md', '')):
                            interview_file = os.path.join(interviews_dir, filename)
                            logger.info(f"Found interview file by reverse prefix match: {filename}")
                            break
    
    # If not found in the user's group, search in all groups
    if not interview_file:
        logger.info(f"Interview file not found in {group_name} group, searching in all groups")
        ...
        for group_dir in os.listdir(data_path):
            ...
            if os.path.isdir(group_path):
                ...
                if os.path.exists(interviews_path) and os.path.isdir(interviews_path):
                    ...
                    logger.info(f"Files in {interviews_path}: {os.listdir(interviews_path) if os.path.exists(interviews_path) else 'directory not found'}")
                    
                    # Try with exact filename first
                    if os.path.exists(os.path.join(interviews_path, original_chat_title)):
                        interview_file = os.path.join(interviews_path, original_chat_title)
                        logger.info(f"Found interview file with exact original name in {group_dir}: {original_chat_title}")
                        break
                    elif os.path.exists(os.path.join(interviews_path, f"{chat_title}.md")):
                        interview_file = os.path.join(interviews_path, f"{chat_title}.md")
                        logger.info(f"Found interview file with exact name in {group_dir}: {chat_title}.md")
                        break
                    else:
                        # First, look for files that START with the chat_title (higher priority)
                        for filename in os.listdir(interviews_path):
                            if filename.endswith('.md'):
                                filename_without_ext = filename.lower().replace('.md', '')
                                if filename_without_ext.startswith(chat_title.lower()):
                                    interview_file = os.path.join(interviews_path, filename)
                                    logger.info(f"Found interview file by prefix match in {group_dir}: {filename}")
                                    break
                        ...
                        # If not found, then look for files that CONTAIN the chat_title (lower priority)
                        for filename in os.listdir(interviews_path):
                            logger.info(f"Checking file in {group_dir}: {filename} against chat_title: {chat_title}")
                            if filename.endswith('.md'):
                                if chat_title.lower() in filename.lower():
                                    interview_file = os.path.join(interviews_path, filename)
                                    logger.info(f"Found interview file by substring match in {group_dir}: {filename}")
                                    break
                                elif chat_title.lower().startswith(filename.lower().replace('.md', '')):
                                    interview_file = os.path.join(interviews_path, filename)
                                    logger.info(f"Found interview file by reverse prefix match in {group_dir}: {filename}")
                                    break
    ...
    if not interview_file:
        logger.warning(f"Interview markdown file not found for chat_title: {chat_title}")
        state['error'] = f"Interview markdown file not found for chat_title: {chat_title}"
        return state
    ...
    try:
        ...
        for filename in test_files:
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(f"# Test Interview: {filename}\n\nThis is a test interview file.")
        ...
        test_cases = [
            ...
            {"chat_title": "Another_Interview-789", "expected_file": "Another_Interview-789.md"},
            {"chat_title": "Interview", "expected_file": "Interview-123.md"},  # Partial match
            {"chat_title": "Test_Interview-456.md", "expected_file": "Test_Interview-456.md"},  # With extension
        ]
        
        # Patch the analyze_interview_markdown function
        with patch('app.services.chat.requirements.requirements_agent_graph.analyze_interview_markdown', 
                  side_effect=modified_analyze_interview_markdown):
            
            for i, test_case in enumerate(test_cases):
                print(colored(f"\nTest case {i+1}: chat_title='{test_case['chat_title']}'", "cyan"))
                ...
                if result_state.get("error"):
                    ...
                    expected_file = os.path.join(interviews_dir, test_case["expected_file"])
                    
                    if found_file == expected_file:
                        print(colored(f"✅ Success: Found the expected file: {os.path.basename(found_file)}", "green"))
                    else:
                        print(colored(f"❌ Error: Found {os.path.basename(found_file)} instead of {test_case['expected_file']}", "red"))
        ...
        try:
            ...
            print(colored(f"Removed directory {os.path.join(settings.CHATBOT_DATA_PATH, group_name)}", "blue"))
        except OSError:
            print(colored(f"Could not remove directory {interviews_dir} (it may not be empty)", "yellow"))
        
    except Exception as e:
        logger.error(f"Error testing interview filename matching: {str(e)}", exc_info=True)
