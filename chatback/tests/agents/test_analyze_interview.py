...
async def test_analyze_interview():
    ...
    try:
        ...
        print(colored(f"Creating requirements agent for session {session_id}...", "blue"))
        ...
        with patch('app.services.chat.requirements.requirements_agent_graph.ChatOpenAI', MockChatOpenAI):
        ...
        if "error" in result:
            ...
            print(colored("1. Make sure you entered the correct filename (including .md extension if needed)", "yellow"))
            print(colored("2. Check that the file exists in one of the group interview directories", "yellow"))
            print(colored("3. Verify that the file has the correct permissions", "yellow"))
        else:
            print(colored("Analysis completed successfully!", "green"))
            print(colored(f"Message: {result.get('message', 'No message')}", "green"))
            print(colored(f"Functional Requirements: {result.get('functional_requirements_path', 'Not available')}", "green"))
            print(colored(f"Non-Functional Requirements: {result.get('non_functional_requirements_path', 'Not available')}", "green"))
            ...
            print(colored(f"Extracted {len(functional_reqs)} functional requirements and {len(non_functional_reqs)} non-functional requirements", "green"))
