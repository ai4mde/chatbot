...
async def test_introduction():
    ...
    try:
        ...
        has_greeting = any(greeting in response for greeting in ["Good morning", "Good afternoon", "Good evening"])
        ...
        if has_greeting and has_agent_name and has_question_prompt:
            ...
            if has_capitalized_username:
                ...
                print(colored("❌ Username capitalization may not be working correctly", "yellow"))
        else:
            ...
            if not has_agent_name:
                print(colored(f"  - Missing agent name ({settings.AGENT_SMITH_NAME})", "red"))
