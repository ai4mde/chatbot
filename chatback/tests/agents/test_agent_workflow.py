...
async def test_langgraph_workflow():
    ...
    try:
        ...
        print(colored("Running LangGraph-based agent workflow with sample conversation...", "blue"))
        start_time = time.time()
        result = await workflow.run_workflow(SAMPLE_CONVERSATION)
        end_time = time.time()
        
        print(colored(f"Agent workflow completed in {end_time - start_time:.2f} seconds!", "green"))
        ...
        if result.get("interview_result"):
            ...
            print(colored(f"Message: {result['interview_result'].get('message', 'N/A')[:100]}...", "cyan"))
        
        # Print diagram result
        if result.get("diagram_result"):
            print(colored("\nDiagram Result:", "blue"))
            print(colored(f"Message: {result['diagram_result'].get('message', 'N/A')}", "cyan"))
            
            if "diagram_files" in result["diagram_result"]:
                print(colored("Diagram Files:", "blue"))
                for diagram_type, file_path in result["diagram_result"]["diagram_files"].items():
        ...
        if result.get("requirements_result"):
            ...
            print(colored(f"Message: {result['requirements_result'].get('message', 'N/A')}", "cyan"))
            
            if "functional_requirements_path" in result["requirements_result"]:
                print(colored(f"Functional Requirements: {result['requirements_result']['functional_requirements_path']}", "cyan"))
            
            if "non_functional_requirements_path" in result["requirements_result"]:
                print(colored(f"Non-Functional Requirements: {result['requirements_result']['non_functional_requirements_path']}", "cyan"))
...
async def test_parallel_execution():
    ...
    try:
        ...
        print(colored("Running direct parallel execution with sample conversation...", "blue"))
        start_time = time.time()
        result = await workflow.run_parallel_workflow(SAMPLE_CONVERSATION)
        end_time = time.time()
        
        print(colored(f"Parallel execution completed in {end_time - start_time:.2f} seconds!", "green"))
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Test the LangGraph-based agent workflow")
    parser.add_argument("--test", choices=["langgraph", "parallel", "all"], default="all",
                        help="Which test to run (default: all)")
    
    args = parser.parse_args()
    
    if args.test == "langgraph" or args.test == "all":
        print(colored("\n=== Testing LangGraph-based Agent Workflow ===", "blue", attrs=["bold"]))
