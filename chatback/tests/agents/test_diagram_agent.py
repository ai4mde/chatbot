...
SAMPLE_CONVERSATION = [
    ...
    {"role": "user", "content": "We need to track tasks, assign them to team members, and monitor progress."},
    {"role": "assistant", "content": "What kind of users will be using the system?"},
    {"role": "user", "content": "We'll have project managers, team members, and administrators."},
    {"role": "assistant", "content": "What specific features do you need?"},
    {"role": "user", "content": "We need task creation, assignment, status updates, file attachments, and reporting."},
    {"role": "assistant", "content": "Any specific technical requirements?"},
    {"role": "user", "content": "It should be a web-based application with mobile support."},
]
...
async def test_analyze_last_chat():
    ...
    try:
        ...
        print(colored(f"Creating diagram agent for analyzing last chat session...", "blue"))
        agent = create_diagram_agent(session_id, username)
...
async def test_save_diagrams():
    ...
    try:
        ...
        print(colored(f"Creating diagram agent for testing save_diagrams_to_directory...", "blue"))
        agent = create_diagram_agent(session_id, username)
...
@enduml"""
        ...
        print(colored(f"Saving diagrams to directory structure: data/{test_group}/diagrams/{test_chat}/", "blue"))
        file_paths = agent.save_diagrams_to_directory(test_diagrams, test_group, test_chat)
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Test the LangGraph-based Diagram Agent")
    parser.add_argument("--test", choices=["generate", "analyze", "save", "all"], default="all",
...
if __name__ == "__main__":
    print(colored("Testing Enhanced LangGraph-based Diagram Agent...", "blue", attrs=["bold"]))
    asyncio.run(main()) 