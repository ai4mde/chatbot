...
from app.services.chat.diagrams import UMLConverterAgent
...
async def test_authentication_and_project(username: str, password: str):
    ...
    try:
        ...
        system_id = await agent.create_system(
            project_id=project_id, name=system_name, description=system_description
        )
        ...
        try:
            ...
            class_lines = [
                line.strip()
                for line in CLASS_DIAGRAM.split("\n")
            ...
            for class_name in class_names:
                ...
                class_content = CLASS_DIAGRAM.split(f"class {class_name}")[1].split(
                    "}"
                )[0]
                ...
                for line in class_content.split("\n"):
                    ...
                    if "(" in line and ")" in line:
                        ...
                        name = (
                            line.split("(")[0].replace("+", "").replace("-", "").strip()
                        )
                ...
                node = await agent.add_node(
                    ...
                    name=class_name,
                    node_type="class",
                    properties={"attributes": attributes, "methods": methods},
                )
        ...
        try:
            ...
            actor_lines = [
                line.strip()
                for line in USE_CASE_DIAGRAM.split("\n")
            ...
            for line in usecase_lines:
                try:
                    ...
                    print(
                        f"✅ Created use case '{name}' (alias: {alias}) with ID: {usecase['data']['id']}"
                    )
                except Exception as e:
            ...
            relationship_lines = [
                line.strip() for line in USE_CASE_DIAGRAM.split("\n") if "-->" in line
            ]
                ...
                try:
                    ...
                    if source in actors:
                        if target not in usecases:
                            print(
                                f"⚠️ Warning: Could not find use case for target {target}"
                            )
                            continue
                        ...
                        print(
                            f"✅ Created association between actor '{source}' and use case '{usecase_aliases[target]}'"
                        )
                    ...
                    # Handle use case relationships (include/extend)
                        ...
                        print(
                            f"✅ Created relationship between use cases '{usecase_aliases[source]}' and '{usecase_aliases[target]}'"
                        )
                    else:
                        print(
                            f"⚠️ Warning: Could not process relationship between {source} and {target}"
                        )
                except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Test ACC API authentication and project creation"
    )
