...
router = APIRouter(
    prefix="/admin", 
    tags=["Admin"],
    dependencies=[Depends(deps.get_current_admin_user)] # Protect all routes in this router
)
    ...
    *, 
    ...
    if user_in.group_id:
        ...
        if not group:
            raise HTTPException(
                status_code=404,
                detail=f"Group with id {user_in.group_id} not found."
            )
...
async def create_group(
    *, 
    ...
    group = await crud.crud_group.get_by_name(db, name=group_in.name) # Assumes get_by_name exists
    if group:
        raise HTTPException(
            status_code=400,
...
async def get_vector_db_status():
    ...
    if not qdrant_manager:
        ...
        return QdrantStatus(connected=False, error="QdrantManager failed to initialize.")
...
@router.post("/vector-db/collections", response_model=QdrantCollectionInfo, status_code=status.HTTP_201_CREATED)
async def create_vector_db_collection(
    *, 
    ...
    try:
        collection_info = qdrant_manager.get_collection_info(collection_name=collection_in.name)
        if collection_info:
            ...
            return QdrantCollectionInfo(
             ...
             logger.error(f"Collection '{collection_in.name}' created but failed to retrieve info immediately.")
             # Return basic info
             return QdrantCollectionInfo(name=collection_in.name, status="unknown")
    except Exception as e:
        logger.error(f"Error retrieving info for newly created collection '{collection_in.name}': {e}", exc_info=True)
        # Return basic info on error
        return QdrantCollectionInfo(name=collection_in.name, status="unknown")
        

@router.delete("/vector-db/collections/{collection_name}", status_code=status.HTTP_204_NO_CONTENT)
