...
async def transcribe_audio(
    *,
    ...
    """
    logger.info(f"Received audio file for transcription from user {current_user.id}. Filename: {file.filename}, Content-Type: {file.content_type}")
    ...
    try:
        ...
        logger.info(f"Successfully transcribed audio for user {current_user.id}. Transcription length: {len(transcribed_text)}")

        return {"transcription": transcribed_text}

    except Exception as e:
        logger.error(f"Error during audio transcription for user {current_user.id}: {e}", exc_info=True)
        raise HTTPException(
