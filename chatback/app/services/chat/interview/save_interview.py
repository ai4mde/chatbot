...
async def save_interview_from_redis(
    ...
    """
    try:
        logger.info(f"Saving interview from Redis for session {session_id} with title {chat_title}")
        ...
        try:
            ...
            logger.info(f"Retrieved {len(messages)} messages from Redis with progress {progress:.1f}%")
            ...
            logger.info(f"Using {len(messages)} messages from state with progress {progress:.1f}%")
        ...
        markdown_content += f"- **Created**: {datetime.now(pytz.UTC).strftime('%Y-%m-%d %H:%M:%S')}\n"
        markdown_content += f"- **Progress**: {progress:.1f}%\n"
        markdown_content += f"- **Status**: {'Completed' if progress >= 100 else 'In Progress'}\n\n"
...
async def get_user_group(username: str, redis_client) -> str:
    ...
    try:
        ...
        with Session() as session:
            ...
            if user:
                if user.group:
                    logger.info(f"Found group for user {username} in database: {user.group.name}")
                    ...
                    if group:
                        logger.info(f"Found group for user {username} in database via group_id: {group.name}")
                        return group.name
            
            logger.warning(f"User {username} not found in database or has no group assigned")
            return 'default'
    except Exception as e:
...
async def get_chat_title_from_db(session_id: str, username: str) -> str:
    ...
    try:
        ...
        logger.info(f"Querying database for chat title for session {session_id} and user {username}")
