...
def load_interview_questions() -> Tuple[Dict[int, str], Dict[int, List[str]]]:
    ...
    try:
        questionnaires_path = os.path.join(settings.CHATBOT_DATA_PATH, "questionnaires", "interview_questions.md")
        
        # Check if file exists
        if not os.path.exists(questionnaires_path):
            logger.error(f"Interview questions file not found at: {questionnaires_path}")
        ...
        if not os.access(questionnaires_path, os.R_OK):
            logger.error(f"Interview questions file is not readable: {questionnaires_path}")
