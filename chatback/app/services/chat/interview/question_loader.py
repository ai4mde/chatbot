...
def load_interview_questions() -> Tuple[Dict[int, str], Dict[int, List[str]]]:
    ...
    try:
        questionnaires_path = os.path.join(
            settings.CHATBOT_DATA_PATH, "questionnaires", "interview_questions.md"
        )

        # Check if file exists
        if not os.path.exists(questionnaires_path):
            logger.error(
                f"Interview questions file not found at: {questionnaires_path}"
            )
            raise FileNotFoundError(
                f"Required file 'interview_questions.md' not found. "
                f"Please ensure it exists at {questionnaires_path}"
            )

        # Check if file is readable
        if not os.access(questionnaires_path, os.R_OK):
            logger.error(
                f"Interview questions file is not readable: {questionnaires_path}"
            )
            raise PermissionError(
                f"Cannot read interview questions file. "
                f"Please check file permissions at {questionnaires_path}"
            )

        with open(questionnaires_path, "r", encoding="utf-8") as f:
            try:
                content = f.read()
        ...
        logger.info(
            f"Successfully loaded {len(sections)} sections with {sum(len(q) for q in questions.values())} questions"
        )
