            ...
            try:
                ...
                for package_name in package_names:
                    try:
                        ...
                        logger.info(f"Using LangGraph version: {langgraph_version} (package: {package_name})")
                        break
                    except importlib.metadata.PackageNotFoundError:
                        continue
                ...
                if not langgraph_version:
                    try:
                        ...
                        if hasattr(langgraph, "__version__"):
                            ...
                            logger.info(f"Using LangGraph version: {langgraph_version} (from module attribute)")
                        elif hasattr(langgraph, "VERSION"):
                            langgraph_version = langgraph.VERSION
                            logger.info(f"Using LangGraph version: {langgraph_version} (from module constant)")
                        else:
                            logger.info("LangGraph module found but version information is not available")
                    except ImportError:
                        logger.warning("Could not import LangGraph module. This is not critical but may be useful for debugging.")
        except Exception as e:
            logger.warning(f"Error determining LangGraph version: {str(e)}. This is not critical.")
            
        logger.info(f"Creating LangGraph-based interview agent for session {session_id}")
