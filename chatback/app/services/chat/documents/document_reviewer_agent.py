...
class DocumentReviewerAgent:
    ...
    def __init__(self, session_id: str, username: str):
        try:
            ...
            self.llm = ChatOpenAI(
                ...
                request_timeout=settings.OPENAI_TIMEOUT or 300, # Use setting or default to 300s
            ...
            self.message_history = RedisChatMessageHistory(
                session_id=f"docreviewer_{session_id}",
                url=redis_url
            )
            ...
            prompts_path = os.path.join(os.path.dirname(__file__), '..', 'prompts', 'review_agent_prompt.yaml')
            try:
                with open(prompts_path, 'r') as f:
                    self.prompts = yaml.safe_load(f)
                logger.info(f"Successfully loaded reviewer prompts from {prompts_path}")
            except FileNotFoundError:
                logger.error(f"Document Reviewer Prompt file not found at {prompts_path}. Please create it.")
                raise
            except yaml.YAMLError as e:
                logger.error(f"Error parsing YAML prompt file {prompts_path}: {e}")
                raise
            
            logger.info(f"DocumentReviewerAgent initialized successfully for session {session_id}")
    ...
    async def review_document(self, 
                             document_path: str, 
                             requirements: Optional[Dict[str, List[Dict[str, str]]]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not review_prompt_config:
                 logger.error("'review_document_prompt' not found in loaded YAML prompts.")
            ...
            review_content = improved_document # The review content *is* the improved doc
    ...
    async def evaluate_documentation_quality(self, document_path: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not eval_prompt_config:
                 logger.error("'evaluate_quality_prompt' not found in loaded YAML prompts.")
