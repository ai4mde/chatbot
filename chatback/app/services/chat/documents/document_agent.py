...
class SRSDocumentAgent(BaseAgent):
    ...
    def __init__(self, session_id: str, username: str):
        ...
        if REDIS_AVAILABLE:
            ...
            logger.warning(
                f"Using fallback chat history for document agent (session: {session_id})"
            )
    ...
    def _get_user_info(self) -> dict:
        ...
        try:
            if not self.redis_client:
                logger.warning(
                    f"Redis client not available, returning default user info"
                )
    ...
    async def generate_srs_document(
        self, chat_name: str, messages: List[BaseMessage], uml_diagrams: str
    ) -> Dict:
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        f"""You are {self.agent_name}, a precise technical writer specializing in Software Requirements Specifications.
                ...
                Be specific and detailed, using actual information from the conversation.""",
                    ...
                    (
                        "human",
                        "Generate SRS content based on this interview: {conversation}",
                    ),
                ]
            )
            ...
            title = chat_name
            description = (
                "Software Requirements Specification based on stakeholder interview"
            )
