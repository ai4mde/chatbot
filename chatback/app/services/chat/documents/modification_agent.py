...
class ModificationAgent:
    ...
    def __init__(self, session_id: str, username: str):
        try:
            ...
            raise ModificationError(
                f"Failed to initialize modification agent: {str(e)}"
            ) from e
    ...
    async def analyze_change_request(self, request: str, current_content: Dict) -> Dict:
        ...
        try:
            ...
            if missing_keys:
                ...
                raise ValueError(
                    f"Current content missing required sections: {', '.join(missing_keys)}"
                )
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        f"""You are {self.agent_name}, the modification specialist.
            ...
            if isinstance(e, (ValueError, ModificationError)):
                raise
            raise ModificationError(
                f"Failed to analyze change request: {str(e)}"
            ) from e
    ...
    async def apply_changes(self, suggestions: Dict, current_content: Dict) -> Dict:
        ...
        try:
            ...
            updated_diagrams = await self._modify_diagrams(
                suggestions, current_content["diagrams"]
            )
            ...
            return {
                "updated_content": {"srs": updated_srs, "diagrams": updated_diagrams},
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        f"""You are {self.agent_name}, applying changes to the SRS document.
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        f"""You are {self.agent_name}, applying changes to UML diagrams.
                ...
                Return the updated PlantUML code for each modified diagram.""",
                    ...
                    (
                        "human",
                        f"""Current diagrams: {json.dumps(current_diagrams)}
            ...
            try:
                ...
                raise ModificationError(
                    "Failed to generate valid diagram modifications"
                )

            # Validate diagram structure
            required_diagrams = ["class", "sequence", "usecase"]
            missing_diagrams = [
                d for d in required_diagrams if d not in modified_diagrams
            ]
            if missing_diagrams:
                raise ModificationError(
                    f"Missing required diagrams: {', '.join(missing_diagrams)}"
                )
