...
class ReviewAgent:
    def __init__(self, session_id: str, username: str):
        try:
            ...
            redis_retry = Retry(ExponentialBackoff(), settings.REDIS_RETRY_ATTEMPTS)
    ...
    async def review_document(
        self, document_content: str, uml_diagrams: str
    ) -> Dict[str, str]:
        ...
        try:
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        f"""You are {self.agent_name}, the senior quality assurance specialist.
                ...
                Format your response as a formal review report.""",
                    ...
                    (
                        "human",
                        "Review this SRS document and UML diagrams:\n\nSRS Document:\n{document}\n\nUML Diagrams:\n{diagrams}",
                    ),
                ]
            )
            ...
            response = await chain.ainvoke(
                {"document": document_content, "diagrams": uml_diagrams}
            )
