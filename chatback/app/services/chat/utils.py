    ...
    try:
        ...
        if user_info_data:
            try:
                ...
                logger.info(f"Found user info for session {session_id} in Redis key '{user_info_key}'")
                # Ensure group_name exists, default if not
                if 'group_name' not in user_info:
                     user_info['group_name'] = 'default'
                     logger.warning(f"'group_name' missing in user_info for session {session_id}, defaulting to 'default'.")
                return user_info
            except json.JSONDecodeError:
                logger.error(f"Failed to decode JSON for user_info key '{user_info_key}'")
                return {'group_name': 'default'} # Default on decode error
            except Exception as e:
                 logger.error(f"Error processing user_info data for session {session_id}: {e}")
                 return {'group_name': 'default'} # Default on other processing errors
        
        logger.warning(f"No user info found in Redis key '{user_info_key}' for session {session_id}")
        ...
        if username_data:
             ...
             logger.warning(f"Found username '{username}' but no user_info object for session {session_id}. Returning default group.")
             return {"username": username, 'group_name': 'default'}
        
        # Final fallback if nothing is found
        logger.error(f"Could not retrieve any user info or username from Redis for session {session_id}")
        return {'group_name': 'default'} # Return default group if nothing found

    except ConnectionError:
        logger.error(f"Redis connection error while getting user info for session {session_id}")
        return {'group_name': 'default'}
    except TimeoutError:
        logger.error(f"Redis timeout while getting user info for session {session_id}")
        return {'group_name': 'default'}
    except Exception as e:
        logger.error(f"Unexpected error getting user info from Redis for session {session_id}: {str(e)}", exc_info=True)
