...
class AgentWorkflow:
    ...
    async def _branch_workflow(self, state: WorkflowState) -> WorkflowState:
        logger.info(
            f"[WORKFLOW] Entering _branch_workflow for session {state['session_id']}"
        )
        ...
        if not next_steps:
            logger.warning(
                "[WORKFLOW] All phases are disabled. Proceeding directly to end."
            )
            next_steps = ["end"]
    ...
    async def _run_diagram_agent(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            state["next_steps"] = [
                "merge"
            ]  # Continue to merge even if there's an error
            return state

    async def _run_requirements_agent(self, state: WorkflowState) -> WorkflowState:
        logger.info(
            f"[WORKFLOW] Entering _run_requirements_agent for session {state['session_id']} with chat_title: {state.get('chat_title')}"
        )
        logger.info(
            f"[WORKFLOW] State at requirements agent: {json.dumps({k: str(v) for k, v in state.items() if k not in ['messages', 'interview_result', 'diagram_result', 'requirements_result', 'uml_converter_result', 'document_result']}, indent=2)}"
        )
    ...
    async def _merge_results(self, state: WorkflowState) -> WorkflowState:
        ...
        requirements_done = (
            "requirements" in state["completed_steps"] or not requirements_enabled
        )

        # Log status of each phase
        logger.info(
            f"Phase status: diagram_enabled={diagram_enabled}, diagram_done={diagram_done}, "
    ...
    async def _end_workflow(self, state: WorkflowState) -> WorkflowState:
        ...
        final_result = {
            ...
            "requirements_result": (
                state["requirements_result"] if not DISABLE_REQUIREMENTS else None
            ),
            "diagram_result": state["diagram_result"] if not DISABLE_DIAGRAM else None,
            "document_result": (
                state["document_result"] if not DISABLE_SRSDOCUMENT else None
            ),
            "completed": True,
            "phase_status": {
                "requirements_enabled": not DISABLE_REQUIREMENTS,
        ...
        logger.info(
            f"Workflow completed with phases: "
    ...
    async def run_workflow(self, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            def log_state(state, step_name):
                logger.info(
                    f"[WORKFLOW] After step {step_name}, state: "
                    + json.dumps(
                        {
                            k: str(v)
            ...
            logger.info(
                f"[WORKFLOW] Final state after workflow: "
                + json.dumps(
                    {
                        k: str(v)
            ...
            return {
                ...
                "requirements_result": (
                    final_state["requirements_result"]
                    if not DISABLE_REQUIREMENTS
                    else None
                ),
                "diagram_result": (
                    final_state["diagram_result"] if not DISABLE_DIAGRAM else None
                ),
                "document_result": (
                    final_state["document_result"] if not DISABLE_SRSDOCUMENT else None
                ),
                "completed": True,
                "phase_status": {
                    "requirements_enabled": not DISABLE_REQUIREMENTS,
            ...
            return {"error": f"Failed to run workflow: {str(e)}", "completed": False}
    ...
    async def run_parallel_workflow(
        ...
        try:
            ...
            results = await asyncio.gather(
                diagram_task, requirements_task, return_exceptions=True
            )
