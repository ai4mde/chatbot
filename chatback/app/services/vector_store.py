...
class QdrantManager:
    def __init__(self):
        ...
        if self.is_connected:
            ...
            self._ensure_collection(self.collection_name) # Ensure the default chat collection exists
    ...
    def _ensure_collection(self, collection_name: str, vector_size: int = 1536, distance: models.Distance = models.Distance.COSINE):
        """Ensure a specific collection exists with the correct settings"""
        if not self.client or not self.is_connected:
             logger.warning(f"Cannot ensure collection '{collection_name}', Qdrant client not connected.")
             return
    ...
    def get_all_collections_info(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for collection_desc in collections_response.collections:
                try:
                    collection_detail = self.client.get_collection(collection_name=collection_desc.name)
                    collections_info.append({
                     ...
                     logger.error(f"Error getting details for collection '{collection_desc.name}': {str(detail_e)}")
                     # Add basic info even if details fail
                     collections_info.append({"name": collection_desc.name, "status": "error", "points_count": None})
    ...
    def create_collection(self, name: str, vector_size: int = 1536, distance: models.Distance = models.Distance.COSINE) -> bool:
        """Create a new collection."""
        if not self.client or not self.is_connected:
            logger.error(f"Cannot create collection '{name}', Qdrant client not connected.")
            return False
        try:
            self.client.create_collection(
            ...
            return True
             ...
             if e.status_code == 400 and "already exists" in str(e).lower():
                 ...
                 logger.error(f"Failed to create Qdrant collection '{name}': {str(e)} - Status: {e.status_code}, Content: {e.content}")
                 return False
        except Exception as e:
            logger.error(f"Unexpected error creating Qdrant collection '{name}': {str(e)}")
            return False

    def delete_collection(self, name: str) -> bool:
        """Delete a collection."""
        if not self.client or not self.is_connected:
            logger.error(f"Cannot delete collection '{name}', Qdrant client not connected.")
            return False
            
        # Prevent deleting the main chat collection through this admin interface?
        if name == self.collection_name:
            logger.warning(f"Attempted to delete the primary chat collection '{name}' via admin interface. Denied.")
        ...
        try:
            ...
            if e.status_code == 404:
                ...
                logger.error(f"Failed to delete Qdrant collection '{name}': {str(e)} - Status: {e.status_code}, Content: {e.content}")
                return False
        except Exception as e:
            logger.error(f"Unexpected error deleting Qdrant collection '{name}': {str(e)}")
            return False
    ...
    def get_collection_info(self, collection_name: Optional[str] = None) -> Optional[models.CollectionInfo]:
        ...
        if not self.client or not self.is_connected:
             logger.warning(f"Cannot get info for collection '{target_collection}', Qdrant client not connected.")
             return None
             
        try:
            collection_info = self.client.get_collection(collection_name=target_collection)
            return collection_info
        except Exception as e:
            # Handle case where collection might not exist yet or other API errors
            logger.error(f"Error getting Qdrant collection info for '{target_collection}': {str(e)}")
            return None
