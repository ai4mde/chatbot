...
class InterviewSessionList(BaseModel):
    sessions: List[InterviewSession] = Field(
...
class DiagramResponse(BaseModel):
    uml_diagrams: str = Field(
        ..., description="Generated UML diagrams in PlantUML format"
    )
...
class ReviewHistory(BaseModel):
    reviews: List[ReviewHistoryEntry] = Field(
        ..., description="List of review history entries"
    )
...
class ModificationHistory(BaseModel):
    modifications: List[ModificationHistoryEntry] = Field(
        ..., description="List of modification history entries"
    )
...
class ModificationRequest(BaseModel):
    ...
    current_content: Dict = Field(
        ..., description="Current document and diagram content"
    )
...
class ModificationImpact(BaseModel):
    srs_changes: List[str] = Field(
        default_factory=list, description="Affected SRS sections"
    )
    diagram_changes: List[str] = Field(
        default_factory=list, description="Affected diagrams"
    )
    dependencies: List[str] = Field(
        default_factory=list, description="Change dependencies"
    )
    risk_level: str = Field(..., description="Risk level of changes")


class ModificationAnalysisResponse(BaseModel):
    suggestions: Dict[str, List[ModificationSuggestion]] = Field(
        ..., description="Suggested modifications"
    )
    impact_analysis: ModificationImpact = Field(
        ..., description="Impact analysis of changes"
    )
